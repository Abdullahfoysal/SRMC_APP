...
public class problemProfileActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private int PREVIOUS_PROBLEM_ID=PROBLEM_ID;
    private Stack<Integer>previousProblem=new Stack<>();
    ...
    private void loadAll(){
        ...
        findId();
        loadUserIntentData();// receiveData();//get problemId,METHOD,SOLVING STRING called once
        loadData();
        loadUserData();
        loadNavMenu();
        setNavMenuInfo();
    }
    private void loadUserIntentData(){
        String problemId="";
        problemId=getIntent().getExtras().getString("problemId");

        PROBLEM_ID= Integer.parseInt(problemId);
        METHOD=getIntent().getExtras().getString("method");
        SOLVING_STRING=getIntent().getExtras().getString("solvingString");
    }
    private void loadUserData(){
        ...
        char ch=solvingString.charAt(PROBLEM_ID);
        ...
        if(result==DbContract.NOT_TOUCHED){
           verdict="No submission";
            ...
            verdict="Enough tried";
            textViewverdict.setTextColor(Color.BLUE);
            submittedSolutionEditText.setHint("Enough Tried");
...
//                listData.add(cursor.getString(0)+". "+cursor.getString(2));
    ...
    void findId(){
        ...
        submitButton= this.<Button>findViewById(R.id.solutionSubmitButtonId);
        previousButton= this.<TextView>findViewById(R.id.previousProblemButtonId);
        nextButton= this.<TextView>findViewById(R.id.nextProblemButtonId);
    ...
    public void onClick(View view) {
        if(view.getId()==R.id.solutionSubmitButtonId){
            ...
            if(realSolution.equals(submittedSolution)){
               ...
               int verdict= DbContract.changeUserSolvingString(PROBLEM_ID,true);
               ...
               // DbContract.Alert(this,"Problem Verdict","Accepted");
                ...
                int verdict= DbContract.changeUserSolvingString(PROBLEM_ID,false);
        ...
        else if(view.getId()==R.id.nextProblemButtonId){

            int nextProblem=DbContract.nextProblemId(METHOD,SOLVING_STRING,PROBLEM_ID);
            if(nextProblem==PROBLEM_ID){
                Toast.makeText(this,"Next doesn't exist",Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            previousProblem.push(PROBLEM_ID);
        ...
        else if(view.getId()==R.id.previousProblemButtonId){

            if(previousProblem.empty()){

                finish();
                return;
            }
            Toast.makeText(this,"Previous problem",Toast.LENGTH_SHORT).show();

            PROBLEM_ID=previousProblem.pop();
            submittedSolution="";
            realSolution="";
